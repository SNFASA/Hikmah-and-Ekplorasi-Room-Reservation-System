    ...
    <div class="header-section mb-4">
        ...
        <div class="header-content d-flex flex-column flex-lg-row justify-content-between align-items-center">
            <div class="header-title">
                ...
                <p class="subtitle">Manage all electronic devices and their configurations</p>
            </div>
            <div class="header-actions mt-3 mt-lg-0">
                <a href="{{ route('electronic.create') }}" class="btn btn-create">
                        ...
                        <div class="col-md-6 text-md-end mt-3 mt-md-0">
                            <div class="stats-info">
                                <span class="badge badge-info">
                                    ...
                                    {{$electronics->count()}} Total Devices
                                </span>
                                <span class="badge badge-success ms-2">
                                    <i class="fas fa-check-circle me-1"></i>
                                    {{$electronics->where('status', 'Active')->count()}} Active
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table modern-table" id="electronicTable">
                            ...
                            <tbody>
                                @foreach($electronics as $device)
                                    <tr class="table-row" data-id="{{$device->no_electronicEquipment}}">
                                        <td class="id-cell">
                                        ...
                                        <td class="status-cell">
                                            <span class="status-badge status-{{ $device->status === 'Active' ? 'success' : 'warning' }}">
                                                <i class="fas fa-{{ $device->status === 'Active' ? 'check-circle' : 'exclamation-triangle' }} me-1"></i>
                                                {{ $device->status }}
                                            </span>
                                        </td>
                                        <td class="action-cell">
                                            <div class="action-buttons">
                                                <a href="{{ route('electronic.edit', $device->no_electronicEquipment) }}"
                                                ...
                                                <form method="POST" action="{{ route('electronic.destroy', $device->no_electronicEquipment) }}" class="d-inline">
                                                    @csrf 
                                                    @method('delete')
                                                    <button type="button"
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <div class="pagination-info">
                                Showing {{$electronics->firstItem()}} to {{$electronics->lastItem()}}
                                of {{$electronics->total()}} results
                            </div>
                        </div>
                        <div class="col-md-6">
            ...
            @else
                ...
                <div class="empty-state">
                    <div class="empty-state-content">
                        ...
                        <h3 class="empty-title">No Electronic Equipment Found</h3>
                        <p class="empty-description">
                            Get started by adding your first electronic device.
                            You can add device details, categories, and status information.
                        </p>
                        <a href="{{ route('electronic.create') }}" class="btn btn-create">
                            <i class="fas fa-plus me-2"></i>
                            Add Electronic
...
$(document).ready(function() {
    ...
    $('.dltBtn').click(function(e) {
        ...
        swal({
            title: "Delete Electronic Device?",
            text: "This action cannot be undone. The electronic device and all its data will be permanently deleted.",
            icon: "warning",
            buttons: {
                cancel: {
                    text: "Cancel",
