        ...
        <form method="post" action="{{ route('room.update', $room->no_room) }}">
          ...
          @method('PATCH') 
                ...
                <ul id="selected-furniture" class="list-group mb-3">
                  @foreach($selectedFurnitures as $selectedFurniture)
                    <li class="list-group-item d-flex justify-content-between align-items-center" data-item-id="{{ $selectedFurniture->no_furniture }}">
                      ...
                      <button type="button" class="btn btn-danger btn-sm remove-item" data-item-id="{{ $selectedFurniture->no_furniture }}" data-type="furniture" aria-label="Remove furniture">&times;</button>
                    </li>
            ...
            <div class="form-group">
                ...
                <ul id="selected-electronics" class="list-group mb-3">
                  ...
                  <li class="list-group-item d-flex justify-content-between align-items-center" data-item-id="{{ $selectedElectronic->no_electronicEquipment }}">
                    ...
                    <button type="button" class="btn btn-danger btn-sm remove-item" data-item-id="{{ $selectedElectronic->no_electronicEquipment }}" data-type="electronics" aria-label="Remove electronic equipment">&times;</button>
                  </li>
            ...
            <div class="form-group d-flex flex-column flex-sm-row justify-content-end gap-2">
                <a href="{{ route('room.index') }}" class="btn btn-warning text-white shadow-sm w-100 w-sm-auto">
            ...
            if (!$(`#selected-furniture li[data-item-id="${selectedItemId}"]`).length) {
                $('#selected-furniture').append(`
                    <li class="list-group-item d-flex justify-content-between align-items-center" data-item-id="${selectedItemId}">
                        ${selectedItemText}
                        <button type="button" class="btn btn-danger btn-sm remove-item" data-item-id="${selectedItemId}" data-type="furniture" aria-label="Remove furniture">&times;</button>
                    </li>
                `);
        ...
        if (selectedItemId) {
            if (!$(`#selected-electronics li[data-item-id="${selectedItemId}"]`).length) {
                $('#selected-electronics').append(`
                    <li class="list-group-item d-flex justify-content-between align-items-center" data-item-id="${selectedItemId}">
                        ${selectedItemText}
                        <button type="button" class="btn btn-danger btn-sm remove-item" data-item-id="${selectedItemId}" data-type="electronics" aria-label="Remove electronic equipment">&times;</button>
                    </li>
                `);
