    ...
    <div class="header-section mb-4">
        ...
        <div class="header-content d-flex flex-column flex-lg-row justify-content-between align-items-center">
            <div class="header-title">
                ...
                <p class="subtitle">Manage all furniture and their configurations</p>
            </div>
            <div class="header-actions mt-3 mt-lg-0">
                <a href="{{ route('ppp.furniture.create') }}" class="btn btn-create">
                        ...
                        <div class="col-md-6 text-md-end mt-3 mt-md-0">
                            <div class="stats-info">
                                <span class="badge badge-info">
                                    ...
                                    {{$furniture->count()}} Total Furniture
                                </span>
                                <span class="badge badge-success ms-2">
                                    <i class="fas fa-check-circle me-1"></i>
                                    {{$furniture->where('status', 'Active')->count()}} Active
                                </span>
                            </div>
                        </div>
                    </div>
                        ...
                        <table class="table modern-table" id="electronicTable">
                            ...
                            <tbody>
                                @foreach($furniture as $item)
                                    <tr class="table-row" data-id="{{$item->no_furniture}}">
                                        <td class="id-cell">
                                        ...
                                        <td class="status-cell">
                                            <span class="status-badge status-{{ $item->status === 'Active' ? 'success' : 'warning' }}">
                                                <i class="fas fa-{{ $item->status === 'Active' ? 'check-circle' : 'exclamation-triangle' }} me-1"></i>
                                                {{ $item->status }}
                                            </span>
                                        </td>
                                        <td class="action-cell">
                                            <div class="action-buttons">
                                                <a href="{{ route('ppp.furniture.edit', $item->no_furniture) }}"
                                                ...
                                                <form method="POST" action="{{ route('ppp.furniture.destroy', $item->no_furniture) }}" class="d-inline">
                                                    @csrf
                                                    @method('delete')
                                                    <button type="button"
                                                            class="action-btn delete-btn dltBtn"
                                                            data-id="{{ $item->no_furniture }}"
                                                            data-bs-toggle="tooltip"
                                                            data-bs-placement="top"
                    ...
                    <div class="row align-items-center">
                        <div class="col-md-6">
                            <div class="pagination-info">
                                Showing {{$furniture->firstItem()}} to {{$furniture->lastItem()}}
                                of {{$furniture->total()}} results
                            </div>
                        </div>
                        <div class="col-md-6">
            ...
            @else
                ...
                <div class="empty-state">
                    <div class="empty-state-content">
                        ...
                        <h3 class="empty-title">No Furniture Equipment Found</h3>
                        <p class="empty-description">
                            Get started by adding your first eurniture device.
                            You can add device details, categories, and status information.
                        </p>
                        <a href="{{ route('ppp.electronic.create') }}" class="btn btn-create">
                            <i class="fas fa-plus me-2"></i>
                            Add Electronic
                        </a>
                    </div>
...
$(document).ready(function() {
    ...
    $('.dltBtn').click(function(e) {
        ...
        swal({
            ...
            text: "This action cannot be undone. The furniture and all its data will be permanently deleted.",
            icon: "warning",
            buttons: {
                cancel: {
                    text: "Cancel",
